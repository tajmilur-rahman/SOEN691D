...
my $j = $dbh_ref->prepare(q{
	...
	from
		(select count(*) p from (select distinct path from dev_area_dev where release=? intersect select distinct path from dev_area_dev where release=?) b) x1,
		(select count(*) t from (select distinct path from dev_area_dev where release=? union select distinct path from dev_area_dev where release=?) c) x2;
});
$get_releases->execute() or die;

print "jaccard similarity between files in two consicutive release periods:\n";
my $i=0;
