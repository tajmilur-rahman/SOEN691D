...
my $j = $dbh_ref->prepare(q{
	...
	from
		(select count(*) p from (select distinct author from dev_area_merge where release=? intersect select distinct author from dev_area_merge where release=?) b) x1,
		(select count(*) t from (select distinct author from dev_area_merge where release=? union select distinct author from dev_area_merge where release=?) c) x2;
});
$get_releases->execute() or die;

print "jaccard similarity between devs in two consicutive merge periods:\n";
my $i=0;
