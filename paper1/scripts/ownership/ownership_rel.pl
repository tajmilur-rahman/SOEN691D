...
my $get_dev_area_rel_info = $dbh_ref->prepare(q{
	select  author, path, release from dev_area_rel
});
...
my $update_ownership_ch = $dbh_ref->prepare(q{
	update dev_area_rel set ownership = ((churn/?)*100)
	where author = ? and path = ? and release = ?
});
my $update_ownership_cm = $dbh_ref->prepare(q{
	update dev_area_rel set ownership = ((commits/?)*100)
	where author = ? and path = ? and release = ?
});

$get_dev_area_rel_info->execute or die;
my $ch0 = 0;
while (my($author, $path, $release) = $get_dev_area_rel_info->fetchrow_array) {
