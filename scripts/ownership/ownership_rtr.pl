...
my $get_dev_area_rtr_info = $dbh_ref->prepare(q{
	select  author, path, release from dev_area_rtr
});
...
my $update_ownership_ch = $dbh_ref->prepare(q{
	update dev_area_rtr set ownership = ((churn/?)*100)
	where author = ? and path = ? and release = ?
});
my $update_ownership_cm = $dbh_ref->prepare(q{
	update dev_area_rtr set ownership = ((commits/?)*100)
	where author = ? and path = ? and release = ?
});

$get_dev_area_rtr_info->execute or die;
my $ch0 = 0;
while (my($author, $path, $release) = $get_dev_area_rtr_info->fetchrow_array) {
