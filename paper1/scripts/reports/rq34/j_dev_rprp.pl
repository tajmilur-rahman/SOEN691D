...
my $j = $dbh_ref->prepare(q{
	...
	from
		(select count(*) p from (select distinct author from dev_area_dev where release=? intersect select distinct author from dev_area_dev where release=?) b) x1,
		(select count(*) t from (select distinct author from dev_area_dev where release=? union select distinct author from dev_area_dev where release=?) c) x2;
});
$get_releases->execute() or die;

print "jaccard similarity between devs in two consicutive release periods:\n";
my $i=0;
